...
public final class MandrillTemplatesApiTest extends MandrillTestCase {
	...
	private static String getTemplateName() {
		return "lutung_templatename_unit_test_" + System.currentTimeMillis();
	}

	@Test(expected = MandrillApiError.class)
	public final void testAddWithoutName() throws IOException, MandrillApiError {
		mandrillApi.templates().add(null, "<html><body><h1>Hello World!</h1></body></html>", null);
		Assert.fail();
	}

	@Test
	public final void testAdd() throws IOException, MandrillApiError {
		final MandrillTemplate t = mandrillApi.templates().add(getTemplateName(),
	...
	public final void testUpdateWithoutName() throws IOException, MandrillApiError {
		mandrillApi.templates().update(null, "<html><body><h1>Hello World!</h1></body></html>", false);
		Assert.fail();
	}

	@Test(expected = MandrillApiError.class)
	public final void testUpdateWithoutCode() throws IOException, MandrillApiError {
		mandrillApi.templates().update(getTemplateName(), null, false);
		Assert.fail();
	}

	@Test
	public final void testUpdate() throws IOException, MandrillApiError {
		MandrillTemplate t = mandrillApi.templates().add(getTemplateName(),
				"<html><body><h1>Hello World!</h1></body></html>", false);
		Assert.assertNotNull(t);
	...
	public final void testTimeSeries() throws IOException, MandrillApiError {
		...
		Assert.assertNotNull(templates);
		if (templates.length > 0) {
			final MandrillTimeSeries[] series = mandrillApi.templates().timeSeries(templates[0].getName());
	...
	public final void testRenderWithoutContent() throws IOException, MandrillApiError {
		...
		mandrillApi.templates().render(getTemplateName(), null, null);
		Assert.fail();
	...
	public final void testRender() throws IOException, MandrillApiError {
		...
		MandrillTemplate t = mandrillApi.templates().add(getTemplateName(), code, false);
		Assert.assertNotNull(t);
		...
		final String rendered = mandrillApi.templates().render(t.getName(), content, mergeVars);
